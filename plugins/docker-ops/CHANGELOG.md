...
- Initial release. Authoring skills: `dockerfile-node` (multi-stage Dockerfile for a Node backend),
  ...
  (project-local `docker/app.compose.yml` wiring `api` + `web`), and `docker-setup` (verify Docker,
  establish `docker/` + `.dockerignore` conventions). Lifecycle skills: `docker-up`, `docker-down`,
